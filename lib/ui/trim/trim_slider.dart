...
class _TrimSliderState extends State<TrimSlider> with AutomaticKeepAliveClientMixin<TrimSlider> {
  ...
  bool get isNotScrollBouncingBack {
    final isBouncingFromLeft = _scrollController.offset < _scrollController.position.minScrollExtent &&
        _scrollController.offset > _lastScrollPixels;
    final isBouncingFromRight = _scrollController.offset > _scrollController.position.maxScrollExtent &&
        _scrollController.offset < _lastScrollPixels;
  ...
  void _updateTrim() {
    if (widget.controller.minTrim != _getRectToTrim(_rect.left) ||
      ...
      if (_isExtendTrim) {
        setState(() {
          _rect = Rect.fromLTWH(_horizontalMargin, _rect.top,
              _geTrimToRect(widget.controller.maxTrim) - _geTrimToRect(widget.controller.minTrim), _rect.height);
        });
        ...
        setState(() {
          _rect = Rect.fromLTRB(_geTrimToRect(widget.controller.minTrim), _rect.top,
              _geTrimToRect(widget.controller.maxTrim), _rect.height);
        });
      }
  ...
  void attachTrimToScroll() {
    ...
    if (isNotScrollBouncingBack) {
      ...
      if (_scrollController.position.extentBefore == 0.0 && _preSynchLeft == null) {
        ...
        final scrollOffset =
            (_scrollController.position.maxScrollExtent - (_lastScrollPixelsBeforeBounce ?? _scrollController.offset))
                .abs();
        _preSynchRight = max(
          0,
    ...
    if (_scrollController.position.extentAfter == 0.0 &&
      ...
      // if view is bouncing on the left side
        ...
        (_preSynchLeft != null || _scrollController.offset.abs() + _horizontalMargin > _rect.left)) {
      _changeTrimRect(
  ...
  void _onHorizontalDragStart(DragStartDetails details) {
    ...
    if (isNotScrollBouncingBack && !_scrollController.position.isScrollingNotifier.value) {
      if (progressTouch.contains(pos)) {
        ...
        rightTouch =
            rightTouch.expandToInclude(Rect.fromLTWH(_rect.right - _edgesTouchMargin, 0, _edgesTouchMargin, 1));
      }
  ...
  void _onHorizontalDragUpdate(DragUpdateDetails details) {
    ...
    switch (_boundary) {
      ...
      case _TrimBoundaries.right:
        ...
        _changeTrimRect(
          width: (_rect.left + _rect.width + delta.dx).clamp(_rect.left, _trimLayout.width + _horizontalMargin) -
              _rect.left,
        );
        break;
  ...
  void _changeTrimRect({double? left, double? width, bool updateTrim = true}) {
    ...
    if (diff < widget.controller.minDuration || diff > widget.controller.maxDuration) {
      if (_boundary == _TrimBoundaries.left) {
        final limitLeft = left.clamp(left + width - _getRectWidthFromDuration(widget.controller.maxDuration),
            left + width - _getRectWidthFromDuration(widget.controller.minDuration));
  ...
  void _resetControllerPosition() async {
    ...
    if (_boundary == null || _boundary == _TrimBoundaries.inside || _boundary == _TrimBoundaries.left) {
      ...
      await widget.controller.betterPlayerController.seekTo(widget.controller.startTrim);
    } else if (_boundary == _TrimBoundaries.right) {
      ...
      await widget.controller.betterPlayerController.seekTo(widget.controller.endTrim);
    }
  }
  ...
  void _controllerSeekTo(double position) async {
    ...
    final to = widget.controller.videoDuration *
        ((position + _scrollController.offset) / (_fullLayout.width + _horizontalMargin * 2));
    await widget.controller.betterPlayerController
        .seekTo(to > widget.controller.endTrim ? widget.controller.endTrim : to);
  }
  ...
  void _updateControllerIsTrimming(bool value) {
    if (value && widget.controller.isPlaying) {
      ...
      widget.controller.betterPlayerController.pause();
    } else if (_isVideoPlayerHold) {
      _isVideoPlayerHold = false;
      widget.controller.betterPlayerController.play();
    }
  ...
  double _getVideoPosition() =>
      ...
      (_fullLayout.width * widget.controller.trimPosition - _scrollController.offset + _horizontalMargin);
  ...
  double _getRectWidthFromDuration(Duration duration) => duration > Duration.zero
      ? _fullLayout.width / (widget.controller.videoDuration.inMilliseconds / duration.inMilliseconds)
      : 0.0;
            ...
            GestureDetector(
              ...
              child: AnimatedBuilder(
                animation: Listenable.merge([
                  widget.controller,
                  widget.controller.betterPlayerController.videoPlayerController,
                ]),
                builder: (_, __) {
